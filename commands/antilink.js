...
async function handleAntilinkCommand(sock, chatId, userMessage, senderId, isSenderAdmin, message) {
    try {
        ...
        if (!action) {
            const usage = `\`\`\`CONFIGURATION ANTILINK\n\n${prefix}antilink on\n${prefix}antilink set delete | kick | warn\n${prefix}antilink off 📴\n\`\`\``;
        ...
        switch (action) {
            case 'on':
                ...
                if (existingConfig?.enabled) {
                    await sock.sendMessage(chatId, { text: '*_🔘Antilink est déjà activé_*' }, { quoted: message });
                    return;
                }
            ...
            case 'off':
                ...
                await sock.sendMessage(chatId, { text: '*_Antilink a été désactivé_*📴' }, { quoted: message });
                break;

            case 'set':
                if (args.length < 2) {
                    await sock.sendMessage(chatId, { 
                        text:`*_Veuillez préciser une action: ${prefix}antilink set delete | kick | warn_*` 
                    }, { quoted: message });
                    return;
                }
                ...
                if (!['delete', 'kick', 'warn'].includes(setAction)) {
                    await sock.sendMessage(chatId, { 
                        text: '*Action non valide. Choisissez supprimer, exclure ou avertir._*' 
                    }, { quoted: message });
                    return;
                }
                const setResult = await setAntilink(chatId, 'on', setAction);
                await sock.sendMessage(chatId, { 
                    text: setResult ? `*_Action Antilink définie sur ${setAction}_*` : '*_Failed to set Antilink action_*' 
                }, { quoted: message });
                break;
